...
working.directory <- "/Users/sranjeva/Desktop/Research/Projects/Higgins_lab/R_code/experiments_4_10_2024"
plotting.directory <- "/Users/sranjeva/Desktop/Research/Projects/Higgins_lab/R_code/experiments_4_10_2024/plots"
...
if(format_data){
dat_key <- read.csv("./Diff_data/MRN_PMRN_key.csv") %>% 
    select(EPIC_PMRN, MGH_MRN) %>% unique()
...
data_lab<- dat_diff %>% 
  select(MRN = MGH_MRN, Seq_Date_Time, Group_Id, Test_Id, Result, Reference_Range) %>% 
...
data_diag = read.csv("../experiments_4_10_2024/MRNs_observation_structure.csv") %>% 
  mutate(Diag_date = as.Date(Diag_date))
...
data_all_old <- read.csv("../experiments_3_20_2024/")
data_all = data3 %>% 
  # filter(!(MRN %in% data_all_old$MRN)) %>% 
  # bind_rows(.,data_all_old) %>% 
  left_join(.,data_diag %>% select(MRN, Diagnosis, Mortality))
}
if(!format_data){
  data_all <- read.csv("./Diff_data/combined_diff_data_4_16.csv")
}
df_stats <- data_all %>% 
...
p <- ggplot(dat_sum %>% filter(t_rel_WBC %in% c(-4:0) & Diagnosis == "MI"), aes(x = t_rel_WBC, y = median)) +
...
data_all <- data_all %>% mutate(Mortality_fctr = recode(as.factor(Mortality), "0" = "Survived", "1" = "Died"))
...
p_var1 <- ggplot(dat_sum   %>% filter(variable == this_variable & t_rel_WBC %in% time_bounds), aes(x = t_rel_WBC, y = median)) +
...
p_var2 <- ggplot(data_all %>% filter(variable == this_variable &t_rel_WBC %in% time_bounds), aes(x = as.factor(t_rel_WBC), y = log(result), fill = Diagnosis)) +
  ...
  xlab("Time relative to  WBC peak")  + 
  ggtitle(paste0(this_variable)) + labs(color = "") + theme(legend.position = "none") + ylim(3.5,4.5)

p_var5 <- ggplot(data_all %>% filter(variable == this_variable &t_rel_WBC %in% time_bounds), aes(x = as.factor(t_rel_WBC), y = log(result), fill = Mortality_fctr)) +
  ...
  ggtitle(paste0(this_variable)) + 
  labs(color = "") +
  theme(legend.position = "none") + ylim(3.5,4.5)


p_var3 <- ggplot(data_all %>% filter(variable == this_variable &t_rel_WBC %in% time_bounds), aes(x = as.factor(t_rel_WBC), y = log(result), fill = Mortality_fctr)) +
  ...
  xlab("Time relative to  WBC peak")  + 
  ggtitle(paste0(this_variable)) + labs(fill = "", color = "") + ylim(3.5,4.5)

p_var3 <- ggplot(data_all %>% filter(variable == this_variable &   Mortality == 0 &t_rel_WBC %in% time_bounds), aes(x = as.factor(Diagnosis), y = log(result), fill = Diagnosis)) +
  ...
  ylab("Log value") +
  xlab("Time relative to  WBC peak")  + 
  ggtitle(paste0(this_variable, ", survivors")) + ylim(3.5,4.5)

p_var4<- ggplot(data_all %>% filter(variable == this_variable &   Mortality == 1 &t_rel_WBC %in% time_bounds), aes(x = as.factor(Diagnosis), y = log(result), fill = Diagnosis)) +
  geom_jitter(aes(color = Diagnosis), alpha = 0.2) +
  geom_boxplot(aes(fill = Diagnosis)) +
  stat_compare_means(comparisons = my_comparisons, size = 2) +
  plot_themes + 
  ...
  ylab("Log value") +
  xlab("Time relative to  WBC peak")  + 
  ggtitle(paste0(this_variable, ", non-survivors")) + ylim(3.5,4.5)

# p_var5 <- ggplot(data_all %>% filter(variable == this_variable & is.finite(result)), 
...
if(save_plots){
  save_plot(paste0("./plots_diff_data/N2_",strsplit(this_variable, c("%"," "))[[1]][1],"_1.pdf"), p_var1, base_width = 8, base_height = 6)
  save_plot(paste0("./plots_diff_data/N2_",strsplit(this_variable, c("%"," "))[[1]][1],"_2.pdf"), p_var2, base_width = 8, base_height = 6)
  save_plot(paste0("./plots_diff_data/N2_",strsplit(this_variable, c("%"," "))[[1]][1],"_3.pdf"), p_var3, base_width = 8, base_height = 6)
  save_plot(paste0("./plots_diff_data/N2_",strsplit(this_variable, c("%"," "))[[1]][1],"_4.pdf"), p_var4, base_width = 8, base_height = 6)
  save_plot(paste0("./plots_diff_data/N2_",strsplit(this_variable, c("%"," "))[[1]][1],"_5.pdf"), p_var5, base_width = 8, base_height = 6)
